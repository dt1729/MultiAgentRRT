...
def time_plot(names):
    ...
    for name in names:
        with open(name) as csv_file:
            ...
            for row in csv_reader:
                data[count].append(float(row[0]))
        print(count, data[count])
        count += 1
    
    # Creating axes instance
    fig1, ax1 = plt.subplots()
    ax1.set_title('Size of Tree Variation')
    ax1.boxplot(data)
    ax1.set_xlabel("Number of Agents")
    ax1.set_ylabel("Size of Tree")
    # show plot
    plt.show()
    ...
    plt.ylabel("Mean Size of Tree")
    plt.title("Mean Size of Tree with increasing agent count")
    plt.show()
